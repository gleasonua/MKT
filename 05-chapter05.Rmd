...
## Definitions of Functions {#sec:Function-Def}
...
```{r machine, echo=FALSE, fig.align='center', out.width='40%', fig.cap='Machine analogy of a function'}
knitr::include_graphics(
  'tikz/function-machine.png',
...
```{definition}
Let $A$ and $B$ be two sets, and let $R$ be a relation from $A$ to $B$. Then $R$ is a function if for every $a\in A$, there exists a unique $b\in B$ such that $(a,b)\in R$. The set $A$ is called the **domain** of the function and $B$ is called the **codomain** of the function.
```
...
```{r, echo=FALSE, fig.align='center', out.width='60%', fig.cap='Concept sketch of $f:A \\rightarrow B$'}
knitr::include_graphics(
  'tikz/function-sets-concept.png',
  auto_pdf = getOption("knitr.graphics.auto_pdf", FALSE),
  dpi = NULL,
  error = getOption("knitr.graphics.error", TRUE)
  )
...
knitr::include_graphics(
  'tikz/graph-ysquared-equals-x.png',
  auto_pdf = getOption("knitr.graphics.auto_pdf", FALSE),
  dpi = NULL,
  error = getOption("knitr.graphics.error", TRUE)
  )
...
$$\left\{ (x,y)\in \mathbb{R}^2 \middle \vert  y=x^2\right\}.$$ This is often represented graphically as 
 ...
 ```{r, echo=FALSE, fig.align='center', out.width='40%', fig.cap=''}
knitr::include_graphics(
  'tikz/graph-y-equals-xsquared.png',
  auto_pdf = getOption("knitr.graphics.auto_pdf", FALSE),
  dpi = NULL,
  error = getOption("knitr.graphics.error", TRUE)
  )
...
```{r squares, echo=FALSE, message=FALSE, warnings=FALSE, results='asis'}
input <- c("0", "1", "2", "3", "$\\vdots$")
output <- c("0", "1", "4", "9", "$\\vdots$")
squaring <- data.frame(input, output)
names(squaring) <- c("$x$", "$y$")
knitr::kable(squaring, caption = 'The squaring function')
```
...
There are some times where it is easier to start describing the function by sketching how elements from the domain map to the co-domain. For example, if the function, $f:\mathbb{N}\rightarrow \mathbb{Z}$, is defined by the relationship
 ...
 ```{r, echo=FALSE, fig.align='center', out.width='30%', fig.cap=''}
knitr::include_graphics(
  'tikz/table-natural-integers.png',
  auto_pdf = getOption("knitr.graphics.auto_pdf", FALSE),
  dpi = NULL,
  error = getOption("knitr.graphics.error", TRUE)
  )
...
knitr::include_graphics(
  'tikz/graph-natural-integers.png',
  auto_pdf = getOption("knitr.graphics.auto_pdf", FALSE),
  dpi = NULL,
  error = getOption("knitr.graphics.error", TRUE)
  )
...
1. Let $A=\{a,b,c\}$ and let $B=\{1,2\}$. List all of the possible functions from $A$ to $B$.
    
1. Define $\mathrm{sum}:\mathbb{N}\times \mathbb{N} \rightarrow \mathbb{N}$ by $(m,n) \mapsto m+n$. Show that $\mathrm{sum}$ is a function.
    
1. Describe how the content of this section relates to the statement, ``You can't take the square root of a negative number.''
    
1. Which of the following are functions? For those that are not functions, explain why not. For those that are functions, find the range of the function.

      a. $f:\mathbb{R} \rightarrow \mathbb{R}$, with $f(x)=\frac{1}{x}$.
      
      a. $\{(x,y)\in \mathbb{R}^2\vert  x^2+y^2=1\}$
      
      a. $g:\mathbb{R}^+ \rightarrow \mathbb{R}$, with $g(x)=\sqrt{x}$.
      
      a. $f:\mathbb{N} \rightarrow \mathbb{N}$, with $h(x)=x^2$.
    
1. Let $f:A\rightarrow B$ be a function. Define a relation on $A$ by $$a_1 \sim_f a_2 \Leftrightarrow f(a_1)=f(a_2)$$ for all $a_1,a_2\in A$.
      
      a. Show that $\sim_f$ is an equivalence relation.
      
      a. If $f:\mathbb{R} \rightarrow \mathbb{R}$ is defined by $f(x)=\sin(2\pi x)$, find the equivalence class of $0$ under this equivalence relation.



## Injections, Surjections, and Bijections {#sec:Bijection}
...
```{definition}
A function $f:A\rightarrow B$ is called an **injection**, or **one-to-one**, if no two elements of the domain map to the same element of the co-domain. Or equivalently, for elements $a_1,a_2 \in A$, $$\left(f(a_1)=f(a_2)\right) \Rightarrow \left(a_1=a_2\right).$$  
```
...
is not one-to-one since $f(1)=f(0)$. If we sketch the graph of this function, we can see that this 
 ...
 ```{r, echo=FALSE, fig.align='center', out.width='30%', fig.cap=''}
knitr::include_graphics(
  'tikz/example2-noninjection.png',
  auto_pdf = getOption("knitr.graphics.auto_pdf", FALSE),
  dpi = NULL,
  error = getOption("knitr.graphics.error", TRUE)
  )
...
is true because if one draws a horizontal line at $y=1$, it intersects the function at two places. This intersection at multiple places by a horizontal line is called the **horizontal line test**.
...
```{definition}
A function $f:A\rightarrow B$ is called a **surjection**, or **onto**, if every element of the co-domain is in the range of $f$. Or equivalently, if for all $b\in B$ there exists a $a\in A$ such that $f(a)=b$.
```
...
The basic example of a function that is not a surjection is given by a function $f:A=\{a\} \rightarrow B=\{b,c\}$ represented by 
 ...
 ```{r, echo=FALSE, fig.align='center', out.width='30%', fig.cap=''}
knitr::include_graphics(
  'tikz/example-nonsurjection.png',
  auto_pdf = getOption("knitr.graphics.auto_pdf", FALSE),
  dpi = NULL,
  error = getOption("knitr.graphics.error", TRUE)
  )
...
```{definition}
A function $f:A\rightarrow B$ is called a **bijection**, or **one-to-one and onto**, if it is both an injection and a surjection. 
```
...
1. For each of the following functions, determine if it is an injection, a surjection, and/or a bijection. 
      
      a. $f:\mathbb{R}\rightarrow \mathbb{R}$ defined by $f(x)=x$.
      
      a. $f:\mathbb{R} \rightarrow \mathbb{R}^+$ defined by $f(x)=|x|$.
      
      a. $f:\mathbb{R}^+ \rightarrow \mathbb{R}$ defined by $f(x)=x^2$.
      
      a. $f:\mathbb{R}\setminus \{0\} \rightarrow \mathbb{R}\setminus \{0\}$ defined by $f(x)=\frac{1}{x}$.
      
      a. $f:\mathbb{N} \rightarrow \mathbb{R}$ defined by $$f(x)=\frac{x}{1+x^2}.$$




## Composition of Functions {#sec:Compositions}
...
```{r function-composition, echo=FALSE, fig.align='center', out.width='100%', fig.cap='Sketch of $f:A \\rightarrow B$, $g:B \\rightarrow C$, and $g \\circ f: A \\rightarrow C$'}
knitr::include_graphics(
...
$$(f\circ g) (b)=b \mbox{ for all }b\in B,$$ then we say that $f$ is **invertible**. 
```
...
We can illustrate this theorem with Figure \@ref(fig:composition-bijections).
 ...
 ```{r composition-bijections, echo=FALSE, fig.align='center', out.width='100%', fig.cap='Composition of Bijections'}
knitr::include_graphics(
  'tikz/function-composition-inverses.png',
  auto_pdf = getOption("knitr.graphics.auto_pdf", FALSE),
  dpi = NULL,
  error = getOption("knitr.graphics.error", TRUE)
  )
...
1. What adjustments can be made to the domain and/or the co-domain of $f:\mathbb{R}\rightarrow \mathbb{R}$ defined by $f(x)=x^2$ in order to make it an invertible function.
    
1. Let $f:A\rightarrow B$ and $g:B\rightarrow C$ be functions.

      a. What properties hold for $g\circ f$ if both $f$ and $g$ are injections?
      
      a. What properties hold for $g\circ f$ if both $f$ and $g$ are surjectionns?
      
      a. What properties hold for $g\circ f$ if $f$ is an injection and $g$ is a surjection?
        
      a. What properties hold for $g\circ f$ if $f$ is a surjection and $g$ in an injection?
...
## Counting and Cardinality {#sec:cardinality}
...
```{r five, echo=FALSE, fig.align='center', out.width='30%', fig.cap='Multiple representations of the number five'}
knitr::include_graphics(
  'images/5.png',
  auto_pdf = getOption("knitr.graphics.auto_pdf", FALSE),
  dpi = NULL,
...
>     a. Understand that the last number name said tells the number of objects counted. The number of objects is the same regardless of their arrangement or the order in which they were counted.
>     a. Understand that each successive number name refers to a quantity that is one larger.


```{definition}
A set $A$ is said to have the same cardinality as the set $B$ if there exists a bijection, $f:A\rightarrow B$.
```
...
We explore this further using subsets of the natural numbers and an idea introduced by David Hilbert in a series of lectures for the general public in 1924 [@Hilbert]. In this example we imagine a hotel with an infinite number of rooms with numbers on the doors, $1, 2, 3, 4, \ldots$, and that every room is full. When someone comes up to the registration desk, the person at the desk says that all the rooms are currently full, but we can make room for one more. The person checking in asks, "How can there be room for one more if all of the rooms are full?" The clerk says, "It's easy. We will ask each person to just gather all of their belongings and they can move to the room with the next higher number." (See Figure \@ref(fig:hilbert1))

```{r hilbert1, echo=FALSE, fig.align='center', out.width='100%', fig.cap="Hilbert's Hotel with one extra room."}
knitr::include_graphics(
  'tikz/Hilbert-Hotel.png',
  auto_pdf = getOption("knitr.graphics.auto_pdf", FALSE),
  dpi = NULL,
  error = getOption("knitr.graphics.error", TRUE)
  )
...
knitr::include_graphics(
  'tikz/table-natural-integers2.png',
  auto_pdf = getOption("knitr.graphics.auto_pdf", FALSE),
  dpi = NULL,
  error = getOption("knitr.graphics.error", TRUE)
  )
...
```{r rational-countable, echo=FALSE, fig.align='center', out.width='100%', fig.cap="Countability argument of non-negative rational numbers"}
knitr::include_graphics(
  'tikz/table-natural-integers3.png',
  auto_pdf = getOption("knitr.graphics.auto_pdf", FALSE),
  dpi = NULL,
  error = getOption("knitr.graphics.error", TRUE)
  )
...
```{r, echo=FALSE, fig.align='center', out.width='30%', fig.cap=""}
knitr::include_graphics(
  'tikz/example-uncountable-bijection.png',
  auto_pdf = getOption("knitr.graphics.auto_pdf", FALSE),
  dpi = NULL,
  error = getOption("knitr.graphics.error", TRUE)
  )
...
1. Prove that the following pairs of sets have the same cardinality.

    a. $3\mathbb{Z} = \{n\in \mathbb{Z}\vert  n=3k \mbox{ for some } k\in \mathbb{Z}\}$ and $5\mathbb{Z} = \{n\in \mathbb{Z}\vert  n=5k \mbox{ for some } k\in \mathbb{Z}\}$
    a. $[a,b]$ and $[c,d]$, where $a$, $b$, $c$, and $d$ are real numbers such that $a<b$ and $c<d$.
    a. $A$ and $A\cup B$, where $A$ and $B$ are countable sets.
    
1. Prove that if $A$ is a set of finite cardinality, the cardinality of the power set of $A$ is $2^{\#(A)}$.
    
1. Look up a proof that the algebraic numbers are countable, and compare that to the proof that the rational numbers are countable.
    
1. Define two sets (not necessarily with finite cardinality) to be equivalent if they have the same cardinality.
